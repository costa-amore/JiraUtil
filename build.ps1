...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "❌ Unit tests failed! Build aborted." -ForegroundColor Red
        Write-Host "Please fix all test failures before building executables." -ForegroundColor Red
        exit 1
    }
    Write-Host "✅ All unit tests passed! Proceeding with version management..." -ForegroundColor Green
} catch {
    Write-Host "❌ Failed to run unit tests: $($_.Exception.Message)" -ForegroundColor Red
