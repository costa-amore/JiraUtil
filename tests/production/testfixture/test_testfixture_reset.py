...
class TestTestFixtureReset(TestJiraUtilsCommand):
    ...
    def test_reset_operation_handles_issues_to_reset(self, mock_get_credentials, mock_jira_class):
        # Given: Issues that need to be reset for rule testing
        mock_jira_instance = self._create_scenario_with_issues_needing_reset_from_spec(mock_get_credentials, mock_jira_class, [
        ...
        self._execute_reset_operation(mock_get_credentials, mock_jira_class, "test-set-label")
    ...
    @patch('testfixture_cli.handlers.JiraInstanceManager')
    @patch('testfixture_cli.handlers.get_jira_credentials')
    def test_reset_operation_handles_jira_connection_failure(self, mock_get_credentials, mock_jira_class):
        ...
        # When: Reset operation is executed
        self._execute_reset_operation(mock_get_credentials, mock_jira_class)
    ...
    @patch('testfixture_cli.handlers.JiraInstanceManager')
    @patch('testfixture_cli.handlers.get_jira_credentials')
    def test_reset_operation_handles_no_issues_found(self, mock_get_credentials, mock_jira_class):
        # Given: Jira instance that returns no issues
        mock_jira_instance = self._create_scenario_with_issues_needing_reset_from_spec(mock_get_credentials, mock_jira_class, [])
    ...
    @patch('testfixture_cli.handlers.JiraInstanceManager')
    @patch('testfixture_cli.handlers.get_jira_credentials')
    def test_reset_operation_skips_issues_already_in_starting_status(self, mock_get_credentials, mock_jira_class):
    ...
    @patch('testfixture_cli.handlers.JiraInstanceManager')
    @patch('testfixture_cli.handlers.get_jira_credentials')
    def test_reset_operation_with_force_update(self, mock_get_credentials, mock_jira_class):
        # Given: Jira manager with test issue that would normally be skipped
        mock_jira_instance = self._create_scenario_with_issues_needing_reset_from_spec(mock_get_credentials, mock_jira_class, [
            {'key': 'PROJ-1', 'current': 'To Do', 'reset_to': 'To Do'}
        ])
        
        # When: Reset operation is executed with force update
        self._execute_reset_operation_with_force_update(mock_get_credentials, mock_jira_class)
    ...
    def _create_scenario_with_issues_needing_reset_from_spec(self, mock_get_credentials, mock_jira_class, issue_specs):
    ...
    def _execute_reset_operation(self, mock_get_credentials, mock_jira_class, test_set_label="test-set-label"):
        self._execute_JiraUtil_with_args(mock_get_credentials, mock_jira_class,
                                       'tf', 'r', '--tsl', test_set_label)

    def _execute_reset_operation_with_force_update(self, mock_get_credentials, mock_jira_class, test_set_label="test-set-label", force_update_via="Done"):
        self._execute_JiraUtil_with_args(mock_get_credentials, mock_jira_class,
                                       'tf', 'r', '--tsl', test_set_label, '--force-update-via', force_update_via)
