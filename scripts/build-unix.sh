...
echo "All tests must pass before building executables"

if ! python tests/run_tests.py; then
    echo "❌ Unit tests failed! Build aborted."
    echo "Please fix all test failures before building executables."
    exit 1
fi
echo "✅ All unit tests passed! Proceeding with version management..."
